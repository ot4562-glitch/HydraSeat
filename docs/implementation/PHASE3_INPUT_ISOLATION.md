...
- Recovery self-test modes cover clean/repeated cycles, lease stall, target kill, watchdog kill/restart, pipe disconnect, adapter loss, abrupt shim-owning process exit, UI-surrogate death, logoff/shutdown notification handling, stale-journal startup blocking, and an external host-kill path. The logoff/shutdown cases exercise the same hidden top-level session-end window used by the interactive host with `WM_QUERYENDSESSION` / `WM_ENDSESSION`; the window procedure only records an atomic stop request and leaves all rollback work to the normal control loop. The Windows `GateCWatchdogRecoveryTests` process test additionally kills the real Gate C host and waits on exact watchdog/target handles to prove no guarded child/helper remains.
...
- The real-acceptance review exposed a Windows lifecycle defect in the original implementation: interactive Gate C uses `user32.dll`, so relying on console-control logoff/shutdown notifications alone is not a trustworthy real-session boundary. The owning packet is reopened while the host adds a fail-closed hidden top-level session-end monitor. Monitor creation must succeed before target activation; `WM_QUERYENDSESSION` / committed `WM_ENDSESSION` only set the atomic stop flag, and the existing control loop performs the durable guarded rollback. Local MSVC Release validation of this repair passes the complete 53/53 CTest suite on both x64 and Win32/x86, including the updated recovery process matrix; exact pushed-SHA CI is still required before returning the packet to `CODE_COMPLETE`.
- Real human Windows acceptance remains pending. Actual Windows logoff plus shutdown/reboot and the declared human-observed desktop-session review have not been performed, so this packet is `IN_PROGRESS` during the lifecycle repair and cannot be `VALIDATED`.
