...
- Real human Windows acceptance still remains pending. In particular, the controlled `CTRL_LOGOFF_EVENT` / `CTRL_SHUTDOWN_EVENT` local handler executions are not actual OS session termination, and actual Windows logoff plus shutdown/reboot and the declared human-observed desktop-session review have not been performed. This packet therefore remains `CODE_COMPLETE`, not `VALIDATED`.
