...
- 2026-08-31 V1 hands-on integration adds native x64 `HydraSeatSetup.exe` as the double-click Install/Repair/Uninstall bootstrapper. It keeps package inspection/non-mutating setup work at normal privilege, requests elevation only for the selected mutation, and delegates to the existing signed PowerShell transaction authority instead of duplicating rollback/ownership semantics. `tools/build_installer_package.ps1`, the signing manifest, artifact preflight and installer-contract validator now treat the bootstrapper as an explicit reviewed release artifact. x86 remains compatibility-build evidence rather than a second host/setup package.
