...
P8-JOURNAL-01 adds the durable recovery-evidence boundary used before later risky activation. A bounded schema-v1 journal records only fixed-width session/lease/generation identity, a deterministic correlation digest of the trusted rollback manifest, ordered transition/action markers, bounded snapshot references, and terminal result. Each durable transition advances by at most one record, native writes use a flushed temporary file plus atomic replacement, and non-clean/corrupt/unreadable startup state enters safe mode instead of permitting automatic activation. The journal is evidence rather than an instruction channel: it cannot create rollback actions, its deterministic digest is not an authentication primitive, and action markers must still match the trusted watchdog manifest. Storage runs synchronously on one serialized control/recovery path, never an input callback. Safe mode clears through this API only after an independently verified rollback has produced a matching clean journal. P8-RESET-01 implements the independent user-facing recovery boundary as standalone `hydra_reset.exe`. Before risky activation, Gate C durably writes a separate bounded/versioned reset registration containing the exact runtime-owner `PID + creationTime100ns` and the same trusted watchdog rollback manifest; its digest covers both owner identity and canonical manifest bytes, so the crash journal remains evidence-only rather than executable authority. Reset validates the registration and manifest, pre-acquires and revalidates exact rollback-target process object handles before stopping the owner, then executes reverse-order rollback through the existing allowlisted registry. This handle preflight is essential when the independently armed watchdog races cleanup after owner death: reset never converts `ACCESS_DENIED`, PID disappearance, PID reuse, or a process-name match into success; it accepts concurrent cleanup only when the already-verified exact process object signals within the bounded action timeout. Failed verification retains registration authority and persists `RecoveryRequired`; even the pre-journal crash window is promoted into a correlated minimal `Preparing` journal for safe retry. The executable has no Qt dependency, process-name kill, arbitrary command/path surface, registry sweep, user-profile deletion, HidHide mutation, or global Raw Input unregister path.

P3-REC-01 wires those two primitives into the controlled Gate C host without expanding the watchdog command surface. Controlled targets are created suspended, exact process identities are captured, the watchdog is armed, and journal prepare records are durably flushed before any target thread resumes. The only registered Gate C recovery action is exact owned-process termination; polling/cursor/Raw Input shim and adapter state remain process-local and disappear with the controlled target process. Active hosts renew the watchdog lease from the control loop only. Clean stop records rollback start, tears controlled targets down in reverse order, verifies their exit, re-establishes the same trusted watchdog plan if the watchdog itself failed, uses watchdog disarm as an idempotent already-satisfied backstop, then records verified clean stop. Host death is handled independently by the watchdog while the journal deliberately remains incomplete so the next startup enters safe mode instead of silently reactivating. UI lifetime remains independent of this authority. Because the interactive host uses `user32`, real desktop-session termination is not entrusted solely to console-control logoff/shutdown notifications: before target activation the host must create a hidden non-activating top-level session-end window on a dedicated message thread. `WM_QUERYENDSESSION` sets only the atomic stop request but does not approve session termination immediately; its message thread waits for a bounded cleanup-complete signal while the existing host control loop performs fast exact-process/watchdog/journal rollback, returns TRUE only after durable cleanup is proven, and returns FALSE on timeout or failed/unproven cleanup. Committed `WM_ENDSESSION` is still recorded as a stop signal. Failure to create the monitor fails closed before activation. Automated native Windows x64/x86 recovery coverage exercises this ordering, and P3-REC-01 is validated by the repaired exact-head CI plus human actual Sign out and actual Restart acceptance with exact-identity cleanup and durable `CleanStop`.

P3-E-01 adds the first bounded external-application compatibility boundary without turning Gate C into a general injector. A declared profile may request only an explicit allowlisted subset of the already-supported polling/cursor/focus/Raw Input IAT hooks through shim ABI v4; legacy V1/V2 installs retain their strict complete-group semantics. `hydra_gate_c_external_harness.exe` can load the fixed bridge only into a process it created suspended itself, after placing that exact process in a kill-on-close Job Object and verifying its architecture. `STARTUPINFOEX` restricts inherited handles to dedicated stdin/stdout/stderr, and exact owned-process handles provide a final cleanup backstop if startup fails before verified Job teardown. The bridge configuration is PID-scoped and contains no arbitrary command. The first measured profile is GLFW 3.5.1 `tests/cursor.c`, pinned to commit `d9d6f0f1f967807ffade6598ea9a631ebaf37a56` with API mask `0x0000b93a`. Because real GLFW consumes focus messages to manage its raw-mouse registration, the profile synthesizes process-local focus lifecycle messages for each controlled window while never mutating the host desktop's global foreground window. A distinct raw-motion warm-up sample is excluded before the four-sample measured window, while direct A/B adapter snapshots independently prove cross-process key-state separation. External source remains outside the repository and exact target bytes/imports are re-verified by the acceptance tooling. Exact code head `12957f0` is validated by PR #24 run `33038227992` across Windows x64/x86, Gate C cross-architecture, and the pinned real GLFW acceptance job.
