...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Seat/workspace configuration (`WorkspaceManager`, Setup UI) | `%LOCALAPPDATA%\\HydraSeat\\workspace_config.json` | two Seat records, Seat names/active state, stable display/keyboard/mouse/controller/audio endpoint IDs, Management Seat; legacy `target_hwnd` key is always persisted as zero | one current file until reconfigured or removed | 1 MiB committed file; one sibling `.tmp` may exist only during an interrupted atomic write | Reconfigure overwrites it; explicit uninstall with `-RemoveHydraSeatUserData` removes the shared LocalAppData HydraSeat root after machine uninstall succeeds | No automatic privacy export | No | Stable hardware IDs stay local; runtime HWND/PID/handle identity is discarded and cannot become share identity | bounded/strict load fails closed; malformed state is not adopted into live configuration; user-state root resolution/creation fails closed instead of falling back to an accidental working directory |
...
The installer `-RemoveHydraSeatUserData` option removes `%LOCALAPPDATA%\\HydraSeat` only **after** machine uninstall succeeds, which now includes `workspace_config.json` together with the other HydraSeat per-user state. It intentionally does not claim to remove the opt-in working-directory `hydraseat_debug.log`; that diagnostics location remains separately documented.
