...
- [ ] Durable crash journal/safe-mode marker is `VALIDATED` (`P8-JOURNAL-01`; fork PR #22 run `32947110442` passes native x64/x86 plus Gate C cross-architecture); Gate C watchdog/crash rollback acceptance is `CODE_COMPLETE` as `P3-REC-01` (implementation head `3d2ec9e`, fork PR #23 run `32957740991`: native x64/x86 53/53 and Gate C cross-architecture pass; committed docs-head `0c24fb0` also reruns local native x64 53/53 plus a 12-case non-disruptive real-process matrix and exact-identity external host-kill/restart refusal with zero HydraSeat orphans), while human-observed actual Windows logoff, shutdown/reboot, and final desktop-session review remain pending
