...
- [ ] Durable crash journal/safe-mode marker is `VALIDATED` (`P8-JOURNAL-01`; fork PR #22 run `32947110442` passes native x64/x86 plus Gate C cross-architecture); Gate C watchdog/crash rollback acceptance is temporarily reopened as `IN_PROGRESS` for `P3-REC-01` after real-acceptance review exposed the interactive `user32` session-end notification gap. The hidden top-level `WM_QUERYENDSESSION` / `WM_ENDSESSION` repair passes local native MSVC x64 and Win32/x86 53/53; exact pushed-SHA x64/x86/cross CI and human-observed actual Windows logoff, shutdown/reboot, and final desktop-session review remain pending
