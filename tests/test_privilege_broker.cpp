...
void testTrustedTypedInstallAndRemove() {
    ...
    check(session.execute(installRequest(), peer(), executor, receipt).succeeded() &&
    ...
    check(session.execute(remove, peer(), executor, receipt).succeeded() &&
...
void testPeerChannelAndReplayAuthorizationFailBeforeExecutor() {
...

    {
        ...
        check(session.execute(installRequest(), wrongUser, executor, receipt).code ==
    ...
    }
    {
        ...
        check(session.execute(installRequest(), wrongChannel, executor, receipt).code ==
    ...
    }
    {
        ...
        check(session.execute(installRequest(), notElevated, executor, receipt).code ==
    ...
    }
    {
        ...
        check(session.execute(installRequest(BrokerResource::RuntimeService, 1u), peer(), executor,
...
void testNoGeneralCommandOrCallerSelectedPathSurface() {
    ...
    check(session.execute(request, peer(), executor, receipt).code ==
    ...
    check(session.execute(request, peer(), executor, receipt).code == BrokerCode::InvalidResource &&
              executor.captureCalls == 0,
    ...
    check(session.execute(request, peer(), executor, receipt).code == BrokerCode::InvalidOperation &&
              executor.captureCalls == 0,
...
void testFailuresRollbackOrEscalateRecovery() {
...

    {
        ...
        const auto result = session.execute(installRequest(), peer(), executor, receipt);
